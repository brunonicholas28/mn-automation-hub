  ...
  do {
    ...
    for (const item of json.items || []) {
      leads.push({
        ...
        // Instantly has moved this key around between API versions. Read all
        // three: a verify pass that silently saw no variables would report a
        // healthy campaign whose emails render with empty merge fields.
        customVariables:
          item.custom_variables || item.personalization_variables || item.payload || null,
      });
    }
    after = json.next_starting_after || null;
...
// The sending accounts, with each one's daily cap and warmup state.
//
// Capacity is a real failure mode on this funnel, not a nicety: nine warm
// mailboxes at 30/day is 270 sends, and a cohort that outgrows that does not
// error - Instantly just carries the remainder into the next sending day,
// which silently stretches a one-day send across the week and desynchronises
// every downstream touch from the cadence day numbering.
//
// Shapes have drifted across Instantly's API versions, so every field is read
// defensively and a mailbox that reports nothing usable is counted as not
// sendable rather than assumed fine.
export async function listAccounts() {
  ...
  do {
    const qs = new URLSearchParams({ limit: "100" });
    if (after) qs.set("starting_after", after);
    const json = await instantly(`/accounts?${qs.toString()}`, { method: "GET" });
    for (const a of json.items || []) {
      const limit = Number(
        a.daily_limit ?? a.sending_limit ?? (a.warmup && a.warmup.limit) ?? 0
      );
      out.push({
        email: String(a.email || "").trim().toLowerCase(),
        status: a.status ?? null,
        active: a.status === 1 || a.status === "active" || a.status === true,
        dailyLimit: Number.isFinite(limit) ? limit : 0,
        warmupStatus: a.warmup_status ?? (a.warmup && a.warmup.status) ?? null,
      });
    }
    after = json.next_starting_after || null;
  } while (after && out.length < 500);
...
//
// Creating a campaign does not start it. Instantly creates one paused, and
// cohort-build deliberately leaves it that way.
//
// Starting one is activateCampaign(), at the bottom of this file. That is a
// change of standing rule, made by Bruno on 2026-09-22 after c20260922 sat
// built and correct in Draft all morning and lost two hours of its sending
// window because pressing Launch was a human step nobody had got to. The rule
// it replaces - "pressing send stays a deliberate human act" - existed to stop
// a half-built cohort emailing real people, so the guard moved rather than
// disappeared: only api/cohort/build.js step=launch calls it, and only after
// runVerify has passed. Nothing else in this repo may call it.
// ---------------------------------------------------------------------------
...
//
// Every other read here answers "is the campaign configured correctly". This
// one answers "what did the recipient receive", which is the question nothing
// asked before 412 emails shipped with a raw spintax subject line on 15 and
// 16 Sep 2026. See lib/jobs/send-audit.js.
//
// Response shapes vary across the v2 endpoints, so this normalises
// defensively and never assumes a field is present.
// ---------------------------------------------------------------------------

function emailBodyText(item) {
  const body = item && item.body;
  if (!body) return "";
  if (typeof body === "string") return body;
  return String(body.text || body.html || "");
}

export async function listSentEmails({ campaignId, limit = 25 } = {}) {
  const qs = new URLSearchParams({ limit: String(Math.min(limit, 100)) });
  if (campaignId) qs.set("campaign_id", campaignId);
  // email_type=sent is the documented filter; if this endpoint ever stops
  // honouring it the caller still filters on direction below.
  qs.set("email_type", "sent");

  const json = await instantly("/emails?" + qs.toString(), { method: "GET" });
  const items = json.items || json.data || [];
  return items.map((item) => ({
    id: item.id || null,
    campaignId: item.campaign_id || item.campaign || null,
    subject: String(item.subject || ""),
    body: emailBodyText(item),
    to: (Array.isArray(item.to_address_email_list)
      ? item.to_address_email_list[0]
      : item.to_address_email_list) || item.lead || null,
    from: item.from_address_email || item.eaccount || null,
    // ue_type 1 is an outbound send on this API; anything else is a reply we
    // did not write and must not judge.
    outbound: item.ue_type === undefined ? true : Number(item.ue_type) === 1,
    sentAt: item.timestamp_created || item.timestamp_email || null,
  }));
}
...
// Starting a campaign.
//
// Read the standing-rule note further up this file before calling this. It
// sends real email to real people as soon as the campaign's sending window is
// open.
//
// Activating an already-active campaign is not reliably a no-op - some
// Instantly versions error instead - so the caller checks the status first and
// skips. This function does not check, on purpose: a caller that has not
// looked at the status has not thought about what it is doing.
//
// The v2 path is not pinned in the docs we hold, and this repo has been bitten
// twice by guessing one (see BLOCKLIST_PATHS and MOVE_PATHS). Same treatment:
// try the plausible paths, report which one answered, and make ok:false a hard
// stop rather than something a caller can mistake for success.
// ---------------------------------------------------------------------------

const ACTIVATE_PATHS = [
  (id) => "/campaigns/" + encodeURIComponent(id) + "/activate",
  (id) => "/campaigns/" + encodeURIComponent(id) + "/launch",
  (id) => "/campaigns/" + encodeURIComponent(id) + "/start",
];

export async function activateCampaign(campaignId) {
  if (!campaignId) throw new Error("activateCampaign needs a campaign id");
  const attempts = [];
  for (const build of ACTIVATE_PATHS) {
    const path = build(campaignId);
    try {
      const json = await instantly(path, { method: "POST", body: {} });
      return { ok: true, path, response: json, attempts };
    } catch (err) {
      attempts.push({ path, error: String(err.message || err).slice(0, 200) });
    }
  }
  return { ok: false, path: null, attempts };
}

// Instantly reports campaign status as a number, and the mapping has been
// stable across every version this repo has seen: 0 draft, 1 active, 2 paused,
// 3 completed, 4 running a subsequence. Anything else is reported as unknown
// rather than assumed safe to start.
export const CAMPAIGN_STATUS_ACTIVE = 1;

export function campaignStatusName(status) {
  const names = { 0: "draft", 1: "active", 2: "paused", 3: "completed", 4: "subsequence" };
  return names[Number(status)] || "unknown(" + String(status) + ")";
}
