...
import { findOrCreatePerson, findDealsByPersonId, createDeal } from "../pipedrive.js";
...
// Was 5. Lowered with the every-6-hours cadence: four runs a day at three pages
// is more throughput than one run a day at five, and a smaller blast radius if
// something goes wrong.
const MAX_PAGES_PER_RUN = Number(process.env.APOLLO_MAX_PAGES || 3);
...
// with it. Deliberately conservative: mint-batch, cohort-build and
// linkedin-shortlist draw on the same daily budget and must not be starved.
const DAILY_BUDGET = Number(process.env.PIPEDRIVE_DAILY_BUDGET || 1200);
const CALLS_PER_CONTACT = 3; // persons/search, deals-by-person, createDeal
const budgetKey = () => `apollo-sync:pd-calls:${new Date().toISOString().slice(0, 10)}`;
...
// Spend is written as it happens rather than at the end, so a run that is
// killed anyway - a slower Pipedrive, a bigger page - still leaves the day's
// counter roughly right instead of silently zero.
const BUDGET_FLUSH_EVERY = 30;
...
export default async function handler(req, res) {
  ...
  const spentToday = Number((await kv.get(budgetKey())) || 0);
  ...
  const flushSpend = async () => {
    const unflushed = pdCalls - flushedCalls;
    if (unflushed <= 0) return;
    const k = budgetKey();
    await kv.incrby(k, unflushed);
    await kv.expire(k, 3 * 24 * 60 * 60);
    flushedCalls = pdCalls;
  };
  ...
  try {
    while (page <= MAX_PAGES_PER_RUN && !reachedCursor) {
      ...
      for (const contact of contacts) {
        ...
        if (pdCalls - flushedCalls >= BUDGET_FLUSH_EVERY) await flushSpend();
