...
async function probeStatusApi() {
  ...
  try {
    res = await fetchWithTimeout(url);
    // Read the WHOLE body before parsing. A ready report's status response
    // carries the full signed token and runs to several kilobytes; truncating
    // it first produces invalid JSON and a false alarm.
    body = await res.text();
  } catch (err) {
    return { status: "unreachable", detail: String(err.message || err).slice(0, 200), url };
  }
  if (looksLikeChallenge(body.slice(0, 2000), res.headers.get("content-type"))) {
    return { status: "blocked", httpStatus: res.status, detail: "HTML challenge page", url };
  }
  if (res.status >= 500)
    return { status: "server_error", httpStatus: res.status, detail: body.slice(0, 300), url };
  try {
  ...
  return { status: "status_api_broken", httpStatus: res.status, detail: body.slice(0, 300), url };
}
...
async function reportStatusFor(rid) {
  try {
    ...
    // Full body, for the same reason as probeStatusApi above: a ready report
    // is kilobytes of signed token, and a 400-character slice of it will never
    // parse. This bug reported a healthy report as a lost lead on the first
    // live sweep, 2026-09-15.
