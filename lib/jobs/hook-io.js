...
// WHAT NEVER LEAVES
//
// The export is committed to a PUBLIC repo, so it carries dealId, company,
// the company's own web domain, and the contact's FIRST NAME. Nothing else.
// No addresses and no surnames.
//
// The domain is there because a bare company name is often unsearchable
// ("Clear", "Fabric", "Strategy"). The first name is there for the
// person-level second pass: when the researcher finds an award for "Melissa
// Guzy at Arbor Ventures", it needs some way to check that is the Melissa we
// are writing to, and not the firm's other partner. Without it the pass has
// to guess who the contact is from the company website, and guessing means
// congratulating the wrong person.
//
// A first name alone is close to useless to anyone scraping this repo for a
// lead list, which a full name plus company would not be. That is the trade,
// made deliberately.
//
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (mode === "export") {
      const rows = leads
        .map((l) => ({
          ...
          firstName: String(l.firstName || "").trim().split(/\s+/)[0] || "",
      ...
      return res.status(200).json({
        ...
        cohort,
        count: rows.length,
        note: "dealId, company, web domain and first name only. No addresses, no surnames.",
        rows,
      });
    }
    ...
    const out = { DRAFT: 0, "NO-TRIGGER": 0, EXCLUDE: 0, REJECT: 0, unmatched: 0, kept: 0 };
    const problems = [];

    // The person-level second pass re-visits leads the company pass already
    // settled, so most of what it sends back is NO-TRIGGER for a lead that is
    // ALREADY NO-TRIGGER. Harmless. What is not harmless is the same thing
    // arriving for a lead that has since gained a good hook: a later pass
    // finding nothing is not evidence the earlier fact was wrong, and letting
    // it overwrite would silently delete verified, sourced research.
    //
    // So a hook is never removed by a pass that found nothing. Only another
    // DRAFT can replace a DRAFT. force=1 overrides, for the rare case where a
    // hook genuinely has to be retracted.
    ...
    for (const raw of rows) {
      ...
      const hadHook = String(lead.hookVerdict || "") === "DRAFT" && String(lead.hook || "").trim();
      if (hadHook && checked.verdict !== "DRAFT" && !force) {
        out.kept += 1;
        continue;
      }
