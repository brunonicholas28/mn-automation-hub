...
// api/cron/run.js dispatches here; the old /api/cron/poll-funnel URL still works
...
import { listCampaigns, listCampaignLeads } from "../instantly.js";
import { listEnrolmentDeals, ENROLMENT_STAGES } from "../pipedrive.js";
import { normaliseCohortId, setCohortFields } from "../cohort.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      const unattributed = { started: 0, completed: 0, booked: 0, held: 0, sold: 0, deals: 0 };
      ...
      for (const d of deals) {
        ...
        // Held and sold are what turn spend into a cost per call and a cost
        // per sale. Same reached-this-stage-or-beyond rule as the rest.
        const held = d.stageId >= ENROLMENT_STAGES.callHeld ? 1 : 0;
        const sold = d.stageId >= ENROLMENT_STAGES.tierSold ? 1 : 0;
        ...
        if (!cohort) {
          ...
          unattributed.held += held;
          unattributed.sold += sold;
          continue;
        }
        ...
        const acc = buckets.get(cohort) || { booked: 0, held: 0, sold: 0 };
        acc.booked += booked;
        acc.held += held;
        acc.sold += sold;
