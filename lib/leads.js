...
export async function putLead(lid, { email, firstName, company, cohort, dealId }) {
...
// Merge fields onto an existing lead without touching the rest of the record.
//
// Used by the hook generator, which writes its verdict, the trigger phrase and
// the source URL onto a lead that already exists. putLead cannot do this: it
// is a create, and calling it again would reset createdAt and wipe whatever
// the funnel has learned about the lead since.
export async function updateLead(lid, patch) {
  const id = normaliseToken(lid);
  if (!id) return false;
  const clean = {};
